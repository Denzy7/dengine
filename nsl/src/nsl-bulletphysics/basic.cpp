...
extern "C" int basic_world_start(void*)
{
    ...
    testnsl = denginescript_nsl_load("./nsl-test.nsl");
    Script scenecam;
    denginescript_nsl_get_script("scenecamera", &scenecam, testnsl);
    ...
    denginescene_ecs_add_script(cam_ent, &scenecam);
    ...
    Entity* plane_ent;
    denginescene_ecs_new_entity(&plane_ent);
    ...
    denginescene_ecs_new_entity(&cube_ent);
    denginescene_ecs_new_meshcomponent(&cube, &cube2_mat, &cube_ent->mesh_component);
    ...
    for(size_t i = 0; i < DENGINE_ARY_SZ(cube_mat_pool); i++)
    {
        ...
        dengine_material_set_shader_shadow(&shadow2d, &cube_mat_pool[i]);

        int pick = dengineutils_rng_int(DENGINE_ARY_SZ(cube_tex_pool));
        Texture* coltex = &cube_tex_pool[pick];
        dengine_material_set_texture(coltex, "diffuseTex", &cube_mat_pool[i]);
    }
    ...
    for(int i = -coef; i < coef; i++)
    {
        ...
        {
            Material* cube_mat = &cube_mat_pool[dengineutils_rng_int(DENGINE_ARY_SZ(cube_mat_pool))];

            Entity* tiny ; denginescene_ecs_new_entity(&tiny);
            ...
            denginescene_ecs_new_meshcomponent(&cube, cube_mat, &tiny->mesh_component);
            btRigidBody* b;
    ...
    glm_vec3_fill(dl_ent->transform.position, -10.0f);
    dl_ent->transform.position[1] = 10.0f;
...
extern "C" int basic_world_update(void*)
{
    ...
    static const char* staticmessageslist[] =
    {
        ...
        "R to reset position",
        "Hold RMB and move mouse for camera position. Use scroll wheel to zoom",
        "",
        "+++ SHADOWS +++",
        "V to toggle shadows",
        "Shadow properties:",
